...
def confusion_all_matrix(threshold=0.4, epoch=0, phase='train', saved=False):
    '''
    ...
    loader = torch.utils.data.DataLoader(
        dataset,
        # sampler=IDS(dataset, callback_get_label=lambda d, i: None),
        shuffle=False,
        batch_size=batchsize,
        num_workers=numwork
    )
    ...
    def _update_backprop_weight(labels, fmask):
        ...
        fixmask = fixmask[:-1]
        fixmask = np.ones(labels.shape, int) - fixmask
    ...
    bp_weight = np.zeros((len(category), 2))
...
if __name__ == "__main__":
    confusion_all_matrix(phase='validate', saved=True)
    # データが存在しなければ生成
